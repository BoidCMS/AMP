...
<html ⚡ lang="<?= $App->get( 'lang' ) ?>">
  <head>
    ...
    <?= $content[1] ?>
    <?php if ( $schema = $App->get_filter( null, 'amp_post_schema' ) ): ?>
      <?= $schema ?>
    <?php else: ?>
  ...
  <body>
    ...
    <header>
      ...
      <?php if ( $App->page( 'thumb' ) ): ?>
        <?= ampify( sprintf( '<img src="%s" alt="Hero" data-hero>', $App->url( 'media/' . $App->page( 'thumb' ) ) ) )[0] ?>
      <?php endif ?>
    </header>
    <article>
      <?= $App->get_action( 'amp_content_top' ) ?>
      <?= $content[0] ?>
      <?= $App->get_action( 'amp_comments' ) ?>
